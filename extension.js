...
async function replaceTextInFiles(filePath, templateName, replaceFileTextFn, renameFileFn, renameSubDirectoriesFn) {
  try {
    const stat = await fs.stat(filePath)
    if (stat.isDirectory()) {
      const files = await fs.readdir(filePath)
      await Promise.all(
        files.map(async (entryFilePath) => {
          return replaceTextInFiles(path.resolve(filePath, entryFilePath), templateName, replaceFileTextFn, renameFileFn, renameSubDirectoriesFn)
        })
      )
      ...
      if (typeof replaceFileTextFn === 'function') {
        await fs.writeFile(filePath, replaceFileTextFn(fileText, templateName, { changeCase, path }))
        /**
         * Rename file
...
async function makeTemplateConfigJs(configFilePath) {
  const defaultConfigFile = (await fs.readFile(path.resolve(__dirname, './assets', 'template.config.js'))).toString('utf8')
...
async function makeSampleTemplate(templateRootPath) {
  const defaultSampleTemplatesPath = path.resolve(__dirname, './assets/.templates')
...
async function createNew(_context, isRenameTemplate) {
  try {
    ...
    const configFilePath = path.resolve(workspaceRootPath, 'template.config.js')
    ...
    const workingPathDir = getWorkingPathDir(_context, vscode.window.activeTextEditor, vscode.workspace)
    ...
    while (variables.length > 0) {
      try {
         ...
         */
        const selectedVariableName = await vscode.window.showQuickPick([...variables, COMMAND_SKIP], {
     ...
     */
    await replaceTextInFiles(dstPath, dstTemplateName, replaceTextWithVariables, replaceTextWithVariables, replaceTextWithVariables)
...
function activate(context) {
  ...
  context.subscriptions.push(
    vscode.commands.registerCommand('extension.createNew', (context) => createNew(context, false)),
    vscode.commands.registerCommand('extension.createNewWithRename', (context) => createNew(context, true))
  )
}
